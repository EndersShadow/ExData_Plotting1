hpc<-read.table("./household_power_consumption.txt", header=TRUE, sep=";", stringsAsFactors=FALSE, dec=".")
subhbc<-hpc[hpc$Date %in% c("1/2/2007","2/2/2007") ,]
gap <- as.numeric(subhbc$Global_active_power)
grp <- as.numeric(subhbc$Global_reactive_power)
volt <- as.numeric(subhbc$Voltage)
submet1 <- as.numeric(subhbc$Sub_metering_1)
submet2 <- as.numeric(subhbc$Sub_metering_2)
submet3 <- as.numeric(subhbc$Sub_metering_3)
png("plot4.png", width=480, height=480)
par(mfrow = c(2, 2)) 
plot(datetime, gap, type="l", xlab="", ylab="Global Active Power", cex=0.2)
plot(datetime, volt, type="l", xlab="datetime", ylab="Voltage")
plot(datetime, submet1, type="l", col="green", ylab="Energy Submetering", xlab="")
lines(datetime, submet2, type="l", col="red")
lines(datetime, submet3, type="l", col="blue")
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=, lwd=2.5, col=c("green", "red", "blue"), bty="o")
plot(datetime, grp, type="l", xlab="datetime", ylab="Global_reactive_power")
dev.off()